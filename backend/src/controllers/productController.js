...
const createProduct = async (req, res) => {
  try {
    ...
    const newProductQuery = `
      ...
      RETURNING *
    `;
    const newProduct = await db.query(newProductQuery, [name, price, description, brand, category, count_in_stock, supplier_id, is_deal_of_the_day, is_flash_sale, is_new_arrival, discounted_price, has_free_delivery, estimated_delivery, colors, sizes, is_highlighted, `{${image_urls.join(',')}}`]);
